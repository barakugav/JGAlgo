...
public class TPMKomlos1985King1997Hagerup2009 implements TPM {
	...
	public <E> Edge<E>[] calcTPM(Graph<E> t, WeightFunction<E> w, int[] queries, int queriesNum) {
		...
		Pair<Graph<Edge<E>>, Integer> r = buildBoruvkaFullyBranchingTree(t, w);
		Graph<Edge<E>> t0 = r.e1;
		int root = r.e2;
		...
		Pair<Edge<Edge<E>>[], int[]> r2 = getEdgeToParentsAndDepth(t0, root);
		Edge<Edge<E>>[] edgeToParent = r2.e1;
		int[] depths = r2.e2;

		int[] q = calcQueriesPerVertex(lcaQueries, depths, edgeToParent);
		Edge<Edge<E>>[][] a = calcAnswersPerVertex(t0, root, q, edgeToParent, t.vertices(), w, bitsTable);
		return extractEdgesFromAnswers(a, q, lcaQueries, depths, w, bitsTable);
	}

	private static <E> Edge<E>[] extractEdgesFromAnswers(Edge<Edge<E>>[][] a, int[] q, int[] lcaQueries, int[] depths,
			WeightFunction<E> w, BitsLookupTable bitsTable) {
	...
	private static <E> Edge<Edge<E>>[][] calcAnswersPerVertex(Graph<Edge<E>> t, int root, int[] q,
			Edge<Edge<E>>[] edgeToParent, int leavesNum, WeightFunction<E> w, BitsLookupTable bitsTable) {
		int n = t.vertices();
		int[] a = new int[n];
		...
		@SuppressWarnings("unchecked")
		Edge<Edge<E>>[][] res = new Edge[leavesNum][];
		...
		Graphs.runDFS(t, root, (v, edgesFromRoot) -> {
			...
			int j = binarySearch(a[v], w.weight(edgeToChild.val()), edgesFromRoot, w, bitsTable);
...
//		return r;
	...
	private static <E> int binarySearch(int av, double weight, List<Edge<Edge<E>>> edgesToRoot, WeightFunction<E> w,
			BitsLookupTable bitsTable) {
		int avsize = bitsTable.bitCount(av);
		if (avsize == 0 || w.weight(edgesToRoot.get(bitsTable.ithBit(av, 0) - 1).val()) < weight)
			return 0;
	...
	private static <E> Pair<Graph<Edge<E>>, Integer> buildBoruvkaFullyBranchingTree(Graph<E> g, WeightFunction<E> w) {
		int n = g.vertices();
		@SuppressWarnings("unchecked")
		...
		for (Graph<Edge<E>> G = Graphs.referenceGraph(g); (n = G.vertices()) > 1;) {
	...
	private static <E> int[] splitQueriesIntoLCAQueries(Graph<Edge<E>> t, int root, int[] queries, int queriesNum) {
	...
	private static <E> Pair<Edge<Edge<E>>[], int[]> getEdgeToParentsAndDepth(Graph<Edge<E>> t, int root) {
		int n = t.vertices();
		@SuppressWarnings("unchecked")
		Edge<Edge<E>>[] edgeToParent = new Edge[n];
	...
	private static <E> int[] calcQueriesPerVertex(int[] lcaQueries, int[] depths, Edge<Edge<E>>[] edgeToParent) {
