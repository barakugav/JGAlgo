...
public abstract class TspAbstract implements Tsp {
	...
	public <V, E> Optional<Path<V, E>> computeShortestTour(Graph<V, E> g, WeightFunction<E> w) {
		if (g instanceof IndexGraph) {
			...
			return computeShortestTour((IndexGraph) g, w0).map(p -> (Path<V, E>) p);

		} else {
			IndexGraph iGraph = g.indexGraph();
			IndexIdMap<E> eiMap = g.indexGraphEdgesMap();
			IWeightFunction iw = IndexIdMaps.idToIndexWeightFunc(w, eiMap);
			Optional<IPath> indexPath = computeShortestTour(iGraph, iw);
			return indexPath.map(p -> Path.pathFromIndexPath(g, p));
		}
	}

	protected abstract Optional<IPath> computeShortestTour(IndexGraph g, IWeightFunction w);
