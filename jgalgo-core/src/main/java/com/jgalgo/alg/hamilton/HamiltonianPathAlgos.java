...
class HamiltonianPathAlgos {
	...
	abstract static class CycleBasedAbstractImpl extends HamiltonianPathAlgos.AbstractImpl {
		...
		public Iterator<IPath> hamiltonianPathsIter(IndexGraph g) {
			 ...
			 * create a graph, g0, with all the vertices and edges of the original graph, and we add another auxillary
			 * vertex x connected to all other vertices. If the graph is directed, x is connected to each vertex with
			 * two edges, one in each direction. If the graph is undirected, x is connected to each vertex with a single
			 * edge. We compute the Hamiltonian cycles in the new graph, and we map each such cycle to an Hamiltonian
			 * path by deleting x from the cycle.
			 */
			...
			return IterTools.map(cyclesIter, cycle -> {
				...
				int target =
						g0.edgeEndpoint(/* targetToXEdge */ cEdges.getInt(xIdx > 0 ? xIdx - 1 : cEdges.size() - 1), x);
		...
		public Iterator<IPath> hamiltonianPathsIter(IndexGraph g, int source, int target) {
			...
			/*
			 * We find all Hamiltonian paths between the given source and target vertices using a reduction to the
			 * problem of finding all Hamiltonian cycles. We create a graph, g0, with all the vertices and edges of the
			 * original graph, and we add another auxillary vertex x, and two additional edges (x, source) (target, x).
			 * We compute the Hamiltonian cycles in the new graph, and we map each such cycle to an Hamiltonian path
			 * between the source and target by deleting the edges (x, source) (target, x), which are require in any
			 * Hamiltonian cycle in g0, from the computed cycle.
			 */
			...
			return IterTools.map(cyclesIter, cycle -> {
				...
				if (g.isDirected()) {
					...
					assert (source == vertexAfterX ? target : source) == g0
							.edgeEndpoint(/* targetToXEdge */ cEdges.getInt(xIdx > 0 ? xIdx - 1 : cEdges.size() - 1),
									x);
				}
