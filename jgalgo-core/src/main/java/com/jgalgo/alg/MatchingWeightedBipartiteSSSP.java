...
class MatchingWeightedBipartiteSSSP extends Matchings.AbstractMaximumMatchingImpl {
	...
	private int[] computeMaxMatching(IndexGraph gOrig, IWeightFunction wOrig, IWeightsBool partition) {
		...
		IndexGraph g = IndexGraphFactory.newDirected().allowSelfEdges().allowParallelEdges().expectedVerticesNum(n + 2)
		...
		IWeightFunction spWeightFunc = ShortestPathUtils.potentialWeightFunc(g, w, potential);
