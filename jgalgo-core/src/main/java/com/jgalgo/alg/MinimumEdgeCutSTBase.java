...
interface MinimumEdgeCutSTBase extends MinimumEdgeCutST {

	@SuppressWarnings("unchecked")
	@Override
	default <V, E> VertexBiPartition<V, E> computeMinimumCut(Graph<V, E> g, WeightFunction<E> w, V source, V sink) {
	...
	@SuppressWarnings("unchecked")
	@Override
	default <V, E> VertexBiPartition<V, E> computeMinimumCut(Graph<V, E> g, WeightFunction<E> w, Collection<V> sources,
			Collection<V> sinks) {
		if (g instanceof IndexGraph) {
			IWeightFunction w0 = WeightFunctions.asIntGraphWeightFunc((WeightFunction<Integer>) w);
	...
	IVertexBiPartition computeMinimumCut(IndexGraph g, IWeightFunction w, int sources, int sinks);
