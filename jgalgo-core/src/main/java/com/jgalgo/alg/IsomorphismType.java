...
public enum IsomorphismType {
	 ...
	 * of \(G_2\) that are not mapped to any vertex of \(G_1\). Full isomorphism between two graphs can be seen as a
	 ...
	 * case of a directed graph, the function must preserve the direction of the edges. The first graph \(G_1\) is the
	 * smaller graph, and the second graph \(G_2\) is the bigger graph, namely there is a sub graph of {@code g2} that
	 * is isomorphic to {@code g1}. Note that subgraph isomorphism can only exists between graphs \(G_1\) and \(G_2\) if
	 * \(|V_1| \leq |V_2|\) and \(|E_1| \leq |E_2|\). There may be vertices of \(G_2\) that are not mapped to any vertex
	 * of \(G_1\), and edges of \(G_2\) that are not mapped to any edge of \(G_1\) (even if they are connecting mapped
	 * vertices of \(G_2\)).
	 */
	SubGraph,
