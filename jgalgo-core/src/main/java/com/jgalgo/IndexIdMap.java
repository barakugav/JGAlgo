 ...
 * view is provided by the {@link Graph#indexGraph()} method, which is a graph in which all methods are accessed with
 ...
 * <p>
 * The mapping interface is used for both vertices and edges, and we use a unify term <i>element</i> in the documentation
 * to describe both of them. If the mapping was obtained by {@link Graph#indexGraphVerticesMap()} it will map between
 * vertices IDs and indices, and if it was obtained by {@link Graph#indexGraphEdgesMap()} it will map between edges IDs
 * and indices.
 *
 * @see    IndexGraph
 * @author Barak Ugav
